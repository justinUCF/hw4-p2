...
{
    return gen_code_ident(vd.ident_list.start, vd.type);
}

// Generate code for the identififers in ident with type vt
// in reverse order (so the first declared are allocated last).
// There are 2 instructions generated for each identifier declared
// (one to allocate space and another to initialize that space)
code_seq gen_code_ident(ident_t ident, type_exp_e vt)
{
    code_seq ret = code_seq_empty();
    ident_t *idp = ident.ident;
    while (idp != NULL) {
	...
	case bool_te:
	    ...
	    break;
	default:
	    bail_with_error("Bad type_exp_e (%d) passed to gen_code_ident!", vt);
	    break;
	}
